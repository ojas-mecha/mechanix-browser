...
class AppLogger {
  ...
  static const _levels = ['debug', 'info', 'warn', 'error', 'none'];
  ...
  static void w(String message, {Object? error, StackTrace? stack}) =>
      _log('warn', message, error: error, stack: stack ?? StackTrace.current);
  ...
  static bool _canLog(String level) {
    ...
    final noneIndex = _levels.indexOf('none');
    return current != -1 && current != noneIndex && target >= current;
  }

  static String _caller() {
    final frames = StackTrace.current.toString().split('\n');
    // frame 0 = _caller, frame 1 = _log, frame 2 = d/i/w/e, frame 3 = actual caller
